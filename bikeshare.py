...
def get_filters():
    ...
    while city not in CITY_DATA.keys():
        print('\n Welcome to this program. Please choose your city:','-'*30,
        '''Acepted input:
        Full name of city, not case sensitive (e.g. chicago or CHICAGO).
        Full name in title case(e.g. Chicago).''','-'*30,sep='\n')

        city = input().lower()

        if city not in CITY_DATA.keys():
            print('''
            \n Please, check your input, it doesn\'t appear to be conforming to any of the acepted input formats.
            Restarting...
            ''')
...
def display_data(df):
    ...
    while rdata not in BIN_RESPONSE_LIST:
        print('\nDo you wish to view the raw data?','-'*30,
            '''Accepted responses:
            Yes or yes
            No or no''','-'*30,sep='\n')
        rdata = input().lower()
